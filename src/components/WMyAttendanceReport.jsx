...
//     "https://bulding-constraction-employee-management.onrender.com/api/workers/Attendance/reports";
//   const SALARY_API =
...
//         const filteredAttendance = attRes.data.data.filter(
...
//         const filteredSalary = salRes.data.data.filter((item) => {
//           if (typeof item?.workerId === "object" && item.workerId !== null) {
//             return (
//               item.workerId?.email?.toLowerCase() === user?.email?.toLowerCase()
//             );
//           } else {
//             return String(item?.workerId) === String(user?._id);
//           }
//         });
//         setSalaryReports(filteredSalary);
//       } else {
//         setSalaryReports([]);
//       }
//     } catch (err) {
...
const WMyAttendanceReports = ({ startDate: propStartDate, endDate: propEndDate }) => {
  const [attendanceReports, setAttendanceReports] = useState([]);
  const [filteredAttendance, setFilteredAttendance] = useState([]);
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const filtered = flattened.filter((item) => {
      ...
      const startValid = startDate ? recordDate.isSameOrAfter(dayjs(startDate), "day") : true;
      const endValid = endDate ? recordDate.isSameOrBefore(dayjs(endDate), "day") : true;
      return recordDate.isValid() && startValid && endValid;
    });
...
export default WMyAttendanceReports;