...
const ContractorSalaryList = () => {
  ...
  // Fetch salaries for logged-in contractor
  ...
  useEffect(() => {
    ...
    data = data.filter((item) => {
      ...
      const searchValid = 
        (item.contractorId?.name?.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ContractorSalaries");
    XLSX.writeFile(wb, "contractor-salary-report.xlsx");
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text("💵 Contractor Salary Report", 40, 40);
    ...
    doc.save("contractor-salary-report.pdf");
            ...
            <tbody>
              {currentSalaries.map((item, idx) => (
                <tr key={item._id}>
                  ...
                  {!isMobile && <td>{item.contractorId?.name || "-"}</td>}
                  <td>
                    {item.date
...
export default ContractorSalaryList;