...
// const WMySalaryReports = ({ startDate, endDate }) => {
...
//             : String(item.workerId) === String(user?._id)
//         );
//         setSalaryReports(filtered);
//       } else {
//         setSalaryReports([]);
//       }
//     } catch (err) {
...
//   // Date filter
//   useEffect(() => {
//     const filtered = salaryReports.filter((item) => {
...
//   // Export Excel
//   const exportExcel = () => {
//     if (!filteredSalary.length) return toast.warning("No data to export.");
//     const ws = XLSX.utils.json_to_sheet(filteredSalary);
...
//     if (!filteredSalary.length) return toast.warning("No data to export.");
//     const doc = new jsPDF();
//     doc.text("💵 Salary Reports", 14, 20);
//     autoTable(doc, {
//       head: [["S.No", "Date", "Paid", "Balance", "Status"]],
//       body: filteredSalary.map((p, i) => [
//         i + 1,
...
//                 ))
//               ) : (
//                 <tr><td colSpan="5" className="text-muted">No salary data</td></tr>
//               )}
//             </tbody>
//           </Table>
//         </div>
...
// export default WMySalaryReports;
...
const WMySalaryReports = () => {
  ...
  const currentReports = filteredSalary
    .filter(item => 
      (item.status?.toLowerCase() || (item.balanceAmount === 0 ? "paid" : "pending")).includes(searchTerm.toLowerCase()) ||
