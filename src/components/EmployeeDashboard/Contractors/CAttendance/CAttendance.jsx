...
// const ContractorAttendanceList = () => {
//   const { user } = useAuth();
...
//         `https://bulding-constraction-employee-management.onrender.com/api/yes/contractors/Attendance?date=${selectedDate}`,
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
//       if (res.data.success && Array.isArray(res.data.data)) {
//         const filtered = res.data.data.filter(att => att.supervisorId?._id === user._id);
//         setAttendances(filtered);
//       } else {
//         toast.warning("⚠️ No attendance records found.");
//         setAttendances([]);
//       }
//     } catch (err) {
...
//         `https://bulding-constraction-employee-management.onrender.com/api/yes/contractors/Attendance/${att._id}`,
//         { status },
...
//     try {
//       const res = await axios.put(
//         "https://bulding-constraction-employee-management.onrender.com/api/contractors/attendance/bulk-by-status",
...
//     presentStatuses.includes(a.currentAttendance?.status || a.status)
//   ).length;
...
//   return (
//     <main className="d-flex justify-content-center px-3">
...
//             <h4 className="fw-bold text-primary mb-0">Welcome Superviosr!</h4>
...
//               />
//             </div>
//             <div className="col-md-6 mt-3 mt-md-0 text-md-end d-flex justify-content-md-end gap-2">
//               <button className="btn btn-success" onClick={() => handleBulkUpdate("Fullday")}>Set Full Day</button>
//               <button className="btn btn-warning" onClick={() => handleBulkUpdate("Halfday")}>Set Half Day</button>
//               <button className="btn btn-primary" onClick={() => handleBulkUpdate("Overtime")}>Set Over Time</button>
//             </div>
//           </div>
...
//                       const status = att.currentAttendance?.status || att.status;
//                       return (
//                         <tr key={att._id}>
//                           <td>{indexOfFirst + index + 1}</td>
//                           <td>{att.name}</td>
//                           <td>{new Date(selectedDate).toLocaleDateString("en-IN")}</td>
//                           <td>{renderStatusBadge(status)}</td>
//                           <td>
//                             <div className="d-flex gap-1 justify-content-center">
//                               <button className="btn btn-sm btn-success" onClick={() => handleUpdate(att, "Fullday")}>FD</button>
//                               <button className="btn btn-sm btn-warning" onClick={() => handleUpdate(att, "Halfday")}>HD</button>
//                               <button className="btn btn-sm btn-primary" onClick={() => handleUpdate(att, "Overtime")}>OT</button>
//                             </div>
//                           </td>
//                         </tr>
//                       );
...
//               disabled={currentPage >= Math.ceil(attendances.length / rowsPerPage)}
//             >
//               Next ➡
//             </button>
//           </div>
...
// export default ContractorAttendanceList;
...
const ContractorName = styled.h5`
  font-weight: 600;
  margin-bottom: 0.25rem;
  color: #2c3e50;
`;

const ContractorId = styled.p`
  font-size: 0.85rem;
  color: #7f8c8d;
  margin-bottom: 0;
`;
...
const ContractorAttendanceList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get(
        `https://bulding-constraction-employee-management.onrender.com/api/yes/contractors/Attendance?date=${dateStr}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const ContractorPhoto = ({ contractor }) => {
    const [imageError, setImageError] = useState(false);
    const photoUrl = contractor.photo
      ? `https://bulding-constraction-employee-management.onrender.com${contractor.photo}`
      : null;
  ...
  const filteredContractors = attendances.filter((contractor) => {
    if (!searchTerm) return true;
    const searchLower = searchTerm.toLowerCase();
    return (
      contractor._id.toString().includes(searchLower) ||
  ...
  return (
    <Container>
      <Header>
        <h2>Contractor Attendance</h2>
        <p>Manage attendance for contractors under your supervision</p>
      </Header>
      ...
      {loading ? (
        ...
        <CardContainer>
          {currentContractors.map((contractor, index) => (
            <AttendanceCard
              key={contractor._id}
              status={contractor.currentAttendance?.status || contractor.status}
            >
              <CardHeader>
                <ContractorPhoto contractor={contractor} />
                <div>
                  <ContractorName>{contractor.name}</ContractorName>
                  <ContractorId>ID: {contractor._id}</ContractorId>
                </div>
              </CardHeader>
              <CardBody>
                <CardField>
                ...
                <CardField>
                  ...
                  <span>
                    {getStatusBadge(
                      contractor.currentAttendance?.status || contractor.status
                    )}
                  </span>
                </CardField>
                ...
                <StatusButtonsContainer>
                  <StatusButton
                    className="fullday"
                    onClick={() => handleUpdate(contractor, "Fullday")}
                  >
                    <FaCheck /> Full Day
                  </StatusButton>
                  <StatusButton
                    className="halfday"
                    onClick={() => handleUpdate(contractor, "Halfday")}
                  >
                    <FaClock /> Half Day
                  </StatusButton>
                  <StatusButton
                    className="overtime"
                    onClick={() => handleUpdate(contractor, "Overtime")}
                  >
                    <FaBusinessTime /> Overtime
                  </StatusButton>
                  <StatusButton
                    className="null"
                    onClick={() => handleUpdate(contractor, null)}
                  >
                    <FaTimes /> Not Marked
                  </StatusButton>
                </StatusButtonsContainer>
      ...
      <PaginationContainer>
        <PageInfo>
          Showing {indexOfFirstContractor + 1} to{" "}
          {Math.min(indexOfLastContractor, filteredContractors.length)} of{" "}
