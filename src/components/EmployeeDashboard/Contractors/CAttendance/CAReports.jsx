...
//   useEffect(() => {
//     if (user?._id) fetchReports(); // ✅ Wait for user to load
//   }, [start, end, user]);

//   const fetchReports = async () => {
//     const url = "https://bulding-constraction-employee-management.onrender.com/api/contractors/Attendance/reports";
...
//           // ✅ Only include records where supervisorId === user._id
//           if (item.supervisorId !== user._id) return;

//           const contractorName = item.name || "Unknown";

//           (item.allRecords || []).forEach((record) => {
//             const raw = record.currentAttendance || record;
//             if (raw && raw.status && raw.date) {
//               const parsedDate = dayjs(raw.date, ["DD/MM/YYYY", "ddd MMM DD YYYY HH:mm:ss ZZ"]);
...
//     autoTable(doc, {
//       startY: 60,
//       head: [["S.No", "Contractor", "Status", "Date"]],
//       body: tableData,
//       styles: { fontSize: 10 },
...
const CAReports = () => {
  ...
  return (
    <Container>
      <Header>
        <h2>Contractor Attendance Reports</h2>
        <p>View and export attendance records for contractors</p>
      </Header>
