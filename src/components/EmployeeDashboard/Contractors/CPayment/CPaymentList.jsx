...
const CPaymentList = () => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      if (res.data.success && Array.isArray(res.data.data)) {
        // Filter payments where the contractor's supervisorId matches the logged-in supervisor
        ...
        if (filtered.length === 0) {
          toast.info("ℹ️ No payment records found for your contractors.");
        }
      } else {
        setPayments([]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await axios.delete(
        `https://bulding-constraction-employee-management.onrender.com/api/yes/contractors/salary/${paymentToDelete._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
    ...
    <Container>
      ...
      >
        ...
        <Modal.Body>
          {selectedPayment && (
            <>
              <h4 className="mb-3">{selectedPayment.contractorId?.name || "Unknown Contractor"}</h4>
              <p className="text-muted mb-4">Contractor</p>
            ...
            {paymentToDelete && (
              <>
                <p className="contractor-name">
                  {paymentToDelete.contractorId?.name || "Unknown Contractor"}
                </p>
      ...
      <Header>
        <h2>Contractor Payment Management</h2>
        <p>View and manage contractor payment records</p>
      </Header>
      ...
      <ActionBar>
        ...
        <ActionButtons>
          <AddButton onClick={() => navigate("/supervisor-dashboard/add-payment")}>
            <FaPlus />
            ...
            {loading ? (
              ...
              currentPayments.map((payment, index) => (
                <tr key={payment._id}>
                  ...
                  <td>
                    {payment.contractorId?.name || "Unknown Contractor"}
                    {isMobile && (
                    ...
                    {isMobile ? (
                      <MobileActionsDropdown>
                        ...
                        <Dropdown.Menu>
                          ...
                          <Dropdown.Item 
                            onClick={() => navigate(`/supervisor-dashboard/edit-payment/${payment._id}`)}
                          >
                            <FaEdit /> Edit
                          </Dropdown.Item>
                      ...
                      <div className="d-flex justify-content-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/supervisor-dashboard/edit-payment/${payment._id}`
                            )
                          }
                          title="Edit"
                        >
              ...
              <tr>
                <td colSpan={isMobile ? 4 : 7} className="text-center py-4">
                  No payment records found for your contractors
                </td>
              </tr>
            )}
          </tbody>
